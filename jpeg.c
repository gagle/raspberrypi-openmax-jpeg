    ...
    case OMX_EventCmdComplete:
      switch (nData1){
        case OMX_CommandStateSet:
          printf ("EVENT: OMX_CommandStateSet, state: %s\n",
              dump_OMX_STATETYPE (nData2));
          wake (component, EventStateSet);
          break;
        case OMX_CommandPortDisable:
          printf ("EVENT: OMX_CommandPortDisable, port: %d\n", nData2);
          wake (component, EventPortDisable);
          break;
        case OMX_CommandPortEnable:
          printf ("EVENT: OMX_CommandPortEnable, port: %d\n", nData2);
          wake (component, EventPortEnable);
          break;
        case OMX_CommandFlush:
          printf ("EVENT: OMX_CommandFlush, port: %d\n", nData2);
          wake (component, EventFlush);
          break;
        case OMX_CommandMarkBuffer:
          printf ("EVENT: OMX_CommandMarkBuffer, port: %d\n", nData2);
          wake (component, EventMarkBuffer);
          break;
      }
      break;
    case OMX_EventError:
      printf ("EVENT: %s\n", dump_OMX_ERRORTYPE (nData1));
      wake (component, EventError);
      break;
    case OMX_EventMark:
      printf ("EVENT: OMX_EventMark\n");
      wake (component, EventMark);
      break;
    case OMX_EventPortSettingsChanged:
      printf ("EVENT: OMX_EventPortSettingsChanged, port: %d\n", nData1);
      wake (component, EventPortSettingsChanged);
      break;
    case OMX_EventParamOrConfigChanged:
      printf ("EVENT: OMX_EventParamOrConfigChanged, nData1: %d, nData2: "
          "%X\n", nData1, nData2);
      wake (component, EventParamOrConfigChanged);
      break;
    case OMX_EventBufferFlag:
      printf ("EVENT: OMX_EventBufferFlag, port: %d\n", nData1);
      wake (component, EventBufferFlag);
      break;
    case OMX_EventResourcesAcquired:
      printf ("EVENT: OMX_EventResourcesAcquired\n");
      wake (component, EventResourcesAcquired);
      break;
    case OMX_EventDynamicResourcesAvailable:
      printf ("EVENT: OMX_EventDynamicResourcesAvailable\n");
...
void enableEncoderOutputPort (
  ...
  if (error = OMX_AllocateBuffer (encoder->handle, encoderBufferOut, 341, NULL,
...
void disableEncoderOutputPort (
  ...
  disablePort (encoder, 341);
  
  //Free encoder output buffer
  printf ("Releasing %s output buffer\n", encoder->name);
  if (error = OMX_FreeBuffer (encoder->handle, 341, encoderBufferOut)){
...
void setCameraSettings (COMPONENT* camera){
  ...
  if (RAW_BAYER){
    ...
    struct {
      ...
      char space[strlen (dummy)];
    } raw;
    OMX_INIT_STRUCTURE (raw.uri);
    raw.uri.nSize = sizeof (raw);
    strcpy (raw.uri.contentURI, dummy);
    if (error = OMX_SetConfig (camera->handle, OMX_IndexConfigCaptureRawImageURI,
        &raw)){
      printf ("ERROR: OMX_SetConfig: %s\n", dump_OMX_ERRORTYPE (error));
      exit (1);
    }
...
void setJpegSettings (COMPONENT* encoder){
  printf ("Configuring JPEG settings\n");
  ...
  OMX_U8 key[] = "IFD0.Make";
  OMX_U8 value[] = "Raspberry Pi";
  struct {
    //These two fields need to be together
    OMX_CONFIG_METADATAITEMTYPE metadata;
    char metadataSpace[strlen (value)];
  } item;
  ...
  item.metadata.nKeySizeUsed = strlen (key);
  strcpy (item.metadata.nKey, key);
  item.metadata.eValueCharset = OMX_MetadataCharsetASCII;
  item.metadata.nValueMaxSize = sizeof (item.metadataSpace);
  item.metadata.nValueSizeUsed = strlen (value);
  strcpy (item.metadata.nValue, value);
  if (error = OMX_SetConfig (encoder->handle, OMX_IndexConfigMetadataItem,
      &item)){
    printf ("ERROR: OMX_SetConfig: %s\n", dump_OMX_ERRORTYPE (error));
    exit (1);
  }
...
int main (){
  ...
  portDefinition.nPortIndex = 72;
  if (error = OMX_GetParameter (camera.handle, OMX_IndexParamPortDefinition,
      &portDefinition)){
    printf ("ERROR: OMX_GetParameter: %s\n", dump_OMX_ERRORTYPE (error));
    exit (1);
  }
  portDefinition.format.image.nFrameWidth  = CAM_WIDTH;
  portDefinition.format.image.nFrameHeight = CAM_HEIGHT;
  ...
  portDefinition.format.image.nStride = CAM_WIDTH;
  ...
  portDefinition.format.image.eColorFormat = OMX_COLOR_FormatUnused;
  ...
  setJpegSettings (&encoder);
  
  //Setup tunnels: camera->image_encode
  ...
  enablePort (&camera, 72);
  enablePort (&encoder, 340);
  ...
  VCOS_UNSIGNED endFlags = EventBufferFlag | EventFillBufferDone;
  VCOS_UNSIGNED retrievedEvents;
  ...
  while (1){
    ...
    wait (&encoder, EventFillBufferDone, &retrievedEvents);
    ...
    if (retrievedEvents == endFlags){
      //Clear the EOS flags
      wait (&camera, EventBufferFlag, NULL);
      wait (&encoder, EventBufferFlag, NULL);
      break;
    }
  }
  ...
  disablePort (&camera, 72);
  disablePort (&encoder, 340);
